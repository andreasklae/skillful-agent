...
def _parse_skill(skill_dir: Path) -> Skill | None:
    ...
    # Discover client-side functions from client_functions.json (optional)
    client_functions: list[ClientFunction] = []
    cf_path = skill_dir / "client_functions.json"
    if cf_path.exists():
        try:
            raw = _json.loads(cf_path.read_text(encoding="utf-8"))
            client_functions = [ClientFunction(**item) for item in raw]
